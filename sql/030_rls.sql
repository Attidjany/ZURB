...
create policy projects_members_select on public.projects for select using (
  owner_id = auth.uid()
  or exists (
    select 1
...
create policy projects_members_update on public.projects for update using (
  owner_id = auth.uid()
  or exists (
...
drop policy if exists blocks_access on public.blocks;
create policy blocks_access on public.blocks for all using (
  exists (
    select 1
    from public.sites s
...
drop policy if exists parcels_access on public.parcels;
create policy parcels_access on public.parcels for all using (
  exists (
    select 1
...
create policy scenarios_select on public.scenarios for select using (
  exists (
    select 1
    from public.sites s
    join public.projects pr on pr.id = s.project_id
...
create policy scenarios_modify on public.scenarios for insert with check (
  exists (
    select 1
    from public.sites s
    join public.projects pr on pr.id = s.project_id
...
create policy scenarios_update on public.scenarios for update using (
  created_by = auth.uid()
  or exists (
    select 1
    from public.sites s
...
drop policy if exists scenario_items_select on public.scenario_items;
create policy scenario_items_select on public.scenario_items for select using (
  exists (
    select 1
...
drop policy if exists scenario_items_modify on public.scenario_items;
create policy scenario_items_modify on public.scenario_items for all using (
  exists (
    select 1
    from public.scenarios sc
    join public.sites s on s.id = sc.site_id
...
create policy cost_params_read on public.cost_params for select using (auth.role() = 'authenticated');

alter table public.mix_rules enable row level security;
drop policy if exists mix_rules_read on public.mix_rules;
create policy mix_rules_read on public.mix_rules for select using (auth.role() = 'authenticated');

alter table public.rents enable row level security;
drop policy if exists rents_read on public.rents;
create policy rents_read on public.rents for select using (auth.role() = 'authenticated');

alter table public.overheads enable row level security;
drop policy if exists overheads_read on public.overheads;
create policy overheads_read on public.overheads for select using (auth.role() = 'authenticated');
