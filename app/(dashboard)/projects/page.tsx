      ...
      <section>
        <ul className={styles.projectGrid}>
          {projects.map((project) => (
            <li className={styles.projectGridItem} key={project!.id}>
              <Link className={styles.projectLink} href={`/projects/${project!.id}/sites`}>
                <strong>{project!.name}</strong>
                <p>{project!.description ?? 'No description yet.'}</p>
              </Link>
            </li>
          ))}
          {projects.length === 0 && <li className={styles.projectGridItem}>No projects yet. Create one above.</li>}
        </ul>
      </section>
    </div>
  );
