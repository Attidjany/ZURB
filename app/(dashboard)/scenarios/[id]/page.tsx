    ...
    <div className={styles.stack}>
      <header className={styles.header}>
        <div className={styles.headerContent}>
          <Link className={styles.headerLink} href={`/sites/${scenario.site_id}/scenarios`}>← Back to scenarios</Link>
          <h1>{scenario.name}</h1>
          <p>Site: {scenario.sites?.name ?? 'Unknown'}</p>
        </div>
        <a className={styles.headerLink} href={`/api/exports/scenario/${scenario.id}/csv`} target="_blank" rel="noopener noreferrer">
          Export CSV
        </a>
      </header>
      <ScenarioPlanner
